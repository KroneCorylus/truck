...
pub enum Error {
    ...
    PathNotSmooth(usize),
    /// an edge of a sweep path that bends more tightly than the profile is wide, so the sweep
    /// would fold over.
    /// cf. [`builder::sweep_along_wire`](../builder/fn.sweep_along_wire.html)
    #[error("edge {0} of the path bends more tightly than the profile is wide.")]
    PathTooTight(usize),
